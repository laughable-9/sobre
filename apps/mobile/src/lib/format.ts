/**
 * Mirrors web/src/lib/format.ts. Pure functions, no React — ported as-is.
 */

import { STROOPS_PER_XLM, type EnvelopeName } from "./config";
import { getPhpPerXlm } from "./usePhpPerXlm";
...
/** "₱1,234.56" — fiat conversion using the live rate, Filipino locale
 *  grouping (fallback in config.ts on cold boot / network failure). */
